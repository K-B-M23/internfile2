# -*- coding: utf-8 -*-
"""
Created on Mon Jun 23 10:35:29 2025
...
diabetes_model=pickle.load(open('D:/internship/savfiles/diabetes_models.sav','rb'))
parkinson_model=pickle.load(open('D:/internship/savfiles/park_models.sav','rb'))
heart_model=pickle.load(open('D:/internship/savfiles/heart_models.sav','rb'))
...
if(selected=='Diabetes Prediction'):
    ...
    dia_diagnosis=''
    if st.button('Result'):
        dia_prediction=diabetes_model([float(Pregnancies),float(Glucose),float(BloodPressure),float(SkinThickness)	,float(Insulin),float(BMI),float(DiabetesPedigreeFunction),float(Age)])
...
if(selected=='Heart Disease Prediction'):
    ...
    heart_diagnosis=''
    if st.button('Result'):
        heart_prediction=([float(age),float(sex),float(cp),float(trestbps),float(chol),float(fbs),float(restecg),float(thalach),float(exang),float(oldpeak),float(slope),float(ca),float(thal)])
...
if(selected=='Parkinsons Prediction'):
    ...
    if st.button('parkinsons Test Result'):
        ...
        if(park_diagnosis[0]==1):
            diagnosis="You have parkinson"
        else:
            diagnosis="You do not have parkinson"        
